...
def verify_multipack_batches(
    ...
    assert (
        len(batches_by_rank) == num_replicas
    ...
    for b in all_batches:
        if padding:
            ...
            assert (
                max(lengths[idx] for idx in b) * len(b) <= max_tokens
            ), "Padded batch * sequence exceeds max_tokens"
        else:
            assert (
                sum(lengths[idx] for idx in b) <= max_tokens
    ...
    assert set(over_limit_indices).issubset(
        not_selected
    ...
    assert (
        total_remaining_tokens <= (num_replicas - 1) * max_tokens
    ), "Tokens remaining that should have been allocated to machines"


@pytest.mark.parametrize("padding", (True, False))
def test_multipack_distributed_batch_sampler(lengths_data, padding: bool):
    batches_by_rank = [
    ...
    verify_multipack_batches(
        batches_by_rank, lengths_data, MAX_LENGTH, NUM_REPLICAS, padding=padding
    )


def test_multipack_distributed_batch_sampler_v2(lengths_data):
    batches_by_rank = [
